...
import { scanImage } from "@/lib/metadata";
import fs from "fs";
...
export async function POST(req: Request) {
  try {
    ...
    // ✅ IMPORTANT: Vercel + vidéos => évite scan lourd qui bloque/hang.
    // MVP: on renvoie meta vide pour la vidéo (ça débloque le flow)
    const meta =
      kind === "image"
        ? await scanImage(inputPath)
        : {
            note: "Video scan skipped (MVP).",
            size: file.size,
            name: file.name,
          };
